...
@export var boosted_roll_speed: float = 400.0 # Gyorsabb roll sebessége (slide után)
@export var slide_speed: float = 250.0
@export var slide_boost: float = 300.0
...
func _physics_process(delta: float):
	...
	if is_sliding:
		...
		if not Input.is_action_pressed("slide") or velocity.length() < 50:
			...
			roll_velocity = direction * boosted_roll_speed
